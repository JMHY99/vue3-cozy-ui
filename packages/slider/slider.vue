<template>
  ...
  >
    ...
    <div v-if="showMarks" class="cozy-slider-marks">
      <span
    ...
    <div v-if="showSteps" class="cozy-slider-steps">
      <span
...
const handleValueChange = (value: number) => {
  ...
  const stepValue =
    Math.round((newValue - props.min) / props.step) * props.step + props.min;
...
const handleTouchStart = (event: TouchEvent) => {
  ...
  event.preventDefault();
  isDragging.value = true;
  startPosition.value = props.vertical
...
const handleTouchMove = (event: TouchEvent) => {
  ...
  const currentPosition = props.vertical
    ? event.touches[0].clientY
    : event.touches[0].clientX;
  const diff = props.vertical
    ? startPosition.value - currentPosition
