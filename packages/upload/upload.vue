<template>
  <div class="cozy-upload" :class="[`cozy-upload-${listType}`]">
    ...
    <div class="cozy-upload-list" v-if="showUploadList && fileList.length">
      ...
      >
        <!-- 图片预览 -->
        <div v-if="listType.startsWith('picture')" class="cozy-upload-list-item-thumbnail">
          <img 
        ...
        <div class="cozy-upload-list-item-info">
          ...
          <span class="cozy-upload-list-item-size">{{ formatSize(file.size) }}</span>
        </div>
...
const props = defineProps({
  ...
  // 上传文件之前的钩子
  beforeUpload: {
    type: Function as PropType<(file: File, fileList: File[]) => boolean | Promise<File>>,
    default: null
  },
  // 限制文件数量
  maxCount: {
...
const upload = async (uploadFile: UploadFile) => {
  if (props.beforeUpload) {
    try {
      const result = await props.beforeUpload(uploadFile.raw as File, fileList.value.map(f => f.raw as File))
      if (result === false) {
