<template>
  <div class="cozy-time-picker" ref="pickerRef">
    <div class="cozy-time-picker-input" @click="togglePanel">
      <input
        ref="inputRef"
        :value="displayValue"
    ...
    <div v-show="panelVisible" class="cozy-time-picker-panel">
      ...
      <div class="cozy-time-picker-footer">
        <button class="cozy-time-picker-btn" @click="selectNow">此刻</button>
        <button
...
const selectedMinute = ref(
  props.modelValue ? props.modelValue.getMinutes() : 0
);
const selectedSecond = ref(
  props.modelValue ? props.modelValue.getSeconds() : 0
);
...
const isDisabledMinute = (minute: number) => {
  return props.disabledMinutes
    ? props.disabledMinutes(minute, selectedHour.value)
    : false;
};

const isDisabledSecond = (second: number) => {
  return props.disabledSeconds
    ? props.disabledSeconds(second, selectedHour.value, selectedMinute.value)
    : false;
};
